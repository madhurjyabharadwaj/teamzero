...
const MatchResults = () => {
  ...
  if (!project) {
    return <div className="min-h-screen"><AmbientBackground /><AppHeader /><main className="container-page py-10"><p className="text-muted-foreground">Loading…</p></main></div>;
  }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="container-page py-10 md:py-14 flex-1 max-w-4xl">
        ...
        <div className="mt-4 flex items-end justify-between gap-4 flex-wrap animate-fade-in">
          ...
          <Button onClick={() => navigate(`/founder/project/${project.id}/shortlist`)} variant="outline" className="border-white/15 bg-white/5 hover:bg-white/10">
            <ListChecks className="h-4 w-4 mr-1.5" /> View shortlist ({invites.length})
          </Button>
        </div>

        {strongCount === 0 && ranked.length > 0 && (
          <GlassCard hover={false} className="mt-6 p-4 border-warning/40 bg-warning/10">
        ...
        <div className="grid gap-4 mt-6">
          {ranked.map((m, i) => {
            ...
            return (
              <Reveal key={m.candidate.id} delay={Math.min(i * 60, 360)}>
                <CandidateCard
                  match={m}
                  action={{
